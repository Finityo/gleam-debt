  ...
  const completeOnboarding = async () => {
    try {
      ...
      await supabase.from("debt_calculator_settings").upsert(
        { user_id: user.id, strategy, extra_monthly: Number(monthlyExtra) || 0 },
        { onConflict: "user_id" }
      );
      ...
      <div className="flex min-h-screen items-center justify-center px-4 py-8">
        <div className="w-full max-w-3xl rounded-3xl border border-border bg-card p-6 shadow-xl md:p-8">
          <div className="flex items-center justify-between gap-2">
            <div>
              <h1 className="text-xl font-semibold">Welcome to Finityo</h1>
          ...
          {error && <div className="mt-3 rounded-lg bg-red-900/60 p-2 text-[11px]">{error}</div>}
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <button
                    ...
                    className={`flex flex-col rounded-2xl border px-3 py-3 text-left text-[11px] ${
                      strategy === "snowball" ? "border-cyan-400 bg-cyan-500/10" : "border-border bg-card"
                    }`}
                  >
                    <div className="text-xs font-semibold">Snowball</div>
                  ...
                  <button
                    ...
                    className={`flex flex-col rounded-2xl border px-3 py-3 text-left text-[11px] ${
                      strategy === "avalanche" ? "border-lime-400 bg-lime-500/10" : "border-border bg-card"
                    }`}
                  >
                    <div className="text-xs font-semibold">Avalanche</div>
                    <p className="mt-1 text-muted-foreground">
            ...
            <div>
              {step > 1 && (
                <button
                  type="button"
                  onClick={back}
                  className="rounded-lg border border-border bg-card px-3 py-1.5 text-[11px]"
                >
                  Back
                </button>
              )}
            ...
            <div className="flex gap-2">
              ...
              {step === 4 && (
                <button
                  ...
                  disabled={saving}
                  className="rounded-lg border border-lime-400/80 bg-lime-500/20 px-3 py-1.5 text-[11px] font-semibold text-lime-100 disabled:opacity-60"
                >
